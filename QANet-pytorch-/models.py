...
class Highway(nn.Module):
    def __init__(self, layer_num: int, size=D):
        super().__init__()
        self.n = layer_num
        self.linear = nn.ModuleList([Initialized_Conv1d(size, size, relu=True, bias=True) for _ in range(self.n)])
...
class Embedding(nn.Module):
    ...
    def forward(self, ch_emb, wd_emb, length):
        N = ch_emb.size()[0]  # (batch, word_size, word_len, char_dim)
        ch_emb = ch_emb.permute(0, 3, 1, 2)  # (batch, char_emb, word_size, word_len)
        ch_emb = F.dropout(ch_emb, p=dropout_char, training=self.training)
        ch_emb = self.conv2d(ch_emb)
        ch_emb = F.relu(ch_emb)
        ch_emb, _ = torch.max(ch_emb, dim=3)
        ch_emb = ch_emb.squeeze()  # batch, max_pool_char_emb, word_size

        wd_emb = F.dropout(wd_emb, p=dropout, training=self.training)  # batch, word_size, word_emb
        wd_emb = wd_emb.transpose(1, 2)   # batch, word_emb, word_size
...
class CQAttention(nn.Module):
    def __init__(self):
        ...
        w4C = torch.empty(D, 1)
        w4Q = torch.empty(D, 1)
        w4mlu = torch.empty(1, 1, D)  # 也可以是torch.randn(D)
    ...
    def forward(self, C, Q, Cmask, Qmask):
        C = C.transpose(1, 2)  # batch, word_size, word_dim
        Q = Q.transpose(1, 2)  # batch, word_size, word_dim
    ...
    def trilinear_for_attention(self, C, Q):
        C = F.dropout(C, p=dropout, training=self.training)  # batch, word_size, word_dim
        Q = F.dropout(Q, p=dropout, training=self.training)
        subres0 = torch.matmul(C, self.w4C).expand([-1, -1, Lq])  # batch, 400, 50
        subres1 = torch.matmul(Q, self.w4Q).transpose(1, 2).expand([-1, Lc, -1])  # batch, 400, 50
        subres2 = torch.matmul(C * self.w4mlu, Q.transpose(1, 2))  # batch, 400, 50
        res = subres0 + subres1 + subres2
        res += self.bias      # 会加在最后一维的所有元素上
        return res
...
class QANet(nn.Module):
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        Cw, Cc = self.word_emb(Cwid), self.char_emb(Ccid)  # batch, word_size, word_emb | batch, word_size, word_len, char_emb
